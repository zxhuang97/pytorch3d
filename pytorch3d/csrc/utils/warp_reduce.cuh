...
template <typename scalar_t>
__device__ void
WarpReduceMin2(scalar_t* min_dists, int64_t* min_idxs, scalar_t*  min_points, const size_t tid) {
    // s = 32
    if (min_dists[tid] > min_dists[tid + 32]) {
        min_idxs[tid] = min_idxs[tid + 32];
        min_dists[tid] = min_dists[tid + 32];
        min_points[tid*3] = min_points[(tid + 32)*3];
        min_points[tid*3+1] = min_points[(tid + 32)*3+1];
        min_points[tid*3+2] = min_points[(tid + 32)*3+2];
    }
    __syncwarp();
    // s = 16
    if (min_dists[tid] > min_dists[tid + 16]) {
        min_idxs[tid] = min_idxs[tid + 16];
        min_dists[tid] = min_dists[tid + 16];
        min_points[tid*3] = min_points[(tid + 16)*3];
        min_points[tid*3+1] = min_points[(tid + 16)*3+1];
        min_points[tid*3+2] = min_points[(tid + 16)*3+2];
    }
    __syncwarp();
    // s = 8
    if (min_dists[tid] > min_dists[tid + 8]) {
        min_idxs[tid] = min_idxs[tid + 8];
        min_dists[tid] = min_dists[tid + 8];
        min_points[tid*3] = min_points[(tid + 8)*3];
        min_points[tid*3+1] = min_points[(tid + 8)*3+1];
        min_points[tid*3+2] = min_points[(tid + 8)*3+2];
    }
    __syncwarp();
    // s = 4
    if (min_dists[tid] > min_dists[tid + 4]) {
        min_idxs[tid] = min_idxs[tid + 4];
        min_dists[tid] = min_dists[tid + 4];
        min_points[tid*3] = min_points[(tid + 4)*3];
        min_points[tid*3+1] = min_points[(tid + 4)*3+1];
        min_points[tid*3+2] = min_points[(tid + 4)*3+2];
    }
    __syncwarp();
    // s = 2
    if (min_dists[tid] > min_dists[tid + 2]) {
        min_idxs[tid] = min_idxs[tid + 2];
        min_dists[tid] = min_dists[tid + 2];
        min_points[tid*3] = min_points[(tid + 2)*3];
        min_points[tid*3+1] = min_points[(tid + 2)*3+1];
        min_points[tid*3+2] = min_points[(tid + 2)*3+2];
    }
    __syncwarp();
    // s = 1
    if (min_dists[tid] > min_dists[tid + 1]) {
        min_idxs[tid] = min_idxs[tid + 1];
        min_dists[tid] = min_dists[tid + 1];
        min_points[tid*3] = min_points[(tid + 1)*3];
        min_points[tid*3+1] = min_points[(tid + 1)*3+1];
        min_points[tid*3+2] = min_points[(tid + 1)*3+2];
    }
    __syncwarp();
}
