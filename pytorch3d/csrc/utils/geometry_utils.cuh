...
__device__ inline thrust::tuple<float3, float3, float3, float3>
PointTriangleClosestPointBackward(
        const float3& p,
        const float3& v0,
        const float3& v1,
        const float3& v2,
    ...
    if ((is_inside) && (norm_normal > kEpsilon)) {
        ...
        if ((e01 <= e02) && (e01 <= e12)) {
            // e01 is smallest
            const auto grads = PointLineClosestPointBackward(p, v0, v1, grad_dist);
            grad_p = thrust::get<0>(grads);
            grad_v0 = thrust::get<1>(grads);
            grad_v1 = thrust::get<2>(grads);
        } else if ((e02 <= e01) && (e02 <= e12)) {
            // e02 is smallest
            const auto grads = PointLineClosestPointBackward(p, v0, v2, grad_dist);
            grad_p = thrust::get<0>(grads);
            grad_v0 = thrust::get<1>(grads);
            grad_v2 = thrust::get<2>(grads);
        } else if ((e12 <= e01) && (e12 <= e02)) {
            // e12 is smallest
            const auto grads = PointLineClosestPointBackward(p, v1, v2, grad_dist);
            grad_p = thrust::get<0>(grads);
            grad_v1 = thrust::get<1>(grads);
            grad_v2 = thrust::get<2>(grads);
        }
